...
// Shopify requires unique setting ids within a section/block, so de-dupe them.
function uniqueSettings(fields) {
  const used = new Set();
  return (fields || []).map((f) => {
    const o = settingJson(f);
    let id = o.id || "setting", i = 2;
    while (used.has(id)) id = `${o.id || "setting"}_${i++}`;
    used.add(id);
    return { ...o, id };
  });
}
...
function buildSectionSchema(node) {
  ...
  schema.settings = uniqueSettings(node.fields);
  if (node.blocks && node.blocks.length) {
    // Block "type" AND "name" must each be unique inside a single section.
    ...
    schema.blocks = node.blocks.map((b) => {
      ...
      usedName.add(name);
      return { type, name, settings: uniqueSettings(b.fields) };
    });
