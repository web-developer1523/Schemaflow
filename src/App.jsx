...
// Valid Shopify resources a metafield can attach to (enum value + Liquid drop name)
const OWNER_TYPES = [
  { value: "PRODUCT", label: "Product", drop: "product" },
  { value: "PRODUCTVARIANT", label: "Variant", drop: "variant" },
  { value: "COLLECTION", label: "Collection", drop: "collection" },
  { value: "CUSTOMER", label: "Customer", drop: "customer" },
  { value: "ORDER", label: "Order", drop: "order" },
  { value: "COMPANY", label: "Company", drop: "company" },
  { value: "PAGE", label: "Page", drop: "page" },
  { value: "BLOG", label: "Blog", drop: "blog" },
  { value: "ARTICLE", label: "Article", drop: "article" },
  { value: "SHOP", label: "Shop", drop: "shop" },
];
const ownerDrop = (v) => (OWNER_TYPES.find((o) => o.value === v) || OWNER_TYPES[0]).drop;
    ...
    .flatMap((n) =>
      n.fields.map((f) => {
        const base = { namespace: "custom", key: handleOf(f.name), name: f.name,
          ownerType: n.ownerType || "PRODUCT", type: FIELD_TYPES[f.type].shopify };
...
function buildLiquid(node, nodes) {
  ...
  if (node.category === "metafield") {
    const owner = ownerDrop(node.ownerType);           // product / customer / collection ...
    L.push(`{%- comment -%}`);
...
function settingJson(s, id) {
  const o = { type: SECTION_TYPES[s.type]?.shopify || "text", id: id || handleOf(s.name), label: s.name };
...
// Assign a unique id to each setting (Shopify requires unique ids per section/block).
function normalizedSettings(fields) {
  const used = new Set();
  return (fields || []).map((f) => {
    const base = handleOf(f.name) || "setting";
    let id = base, i = 2;
...
// Assign a unique type + name to each block, with its normalized settings.
function normalizedBlocks(node) {
  const usedType = new Set();
  const usedName = new Set();
  return (node.blocks || []).map((b) => {
    const baseT = handleOf(b.name) || "block";
    let type = baseT, i = 2;
    while (usedType.has(type)) type = `${baseT}_${i++}`;
    usedType.add(type);
    const baseN = b.name || "Block";
    let name = baseN, j = 2;
    while (usedName.has(name)) name = `${baseN} ${j++}`;
    usedName.add(name);
    return { type, name, settings: normalizedSettings(b.fields) };
  });
}
...
function buildSectionSchema(node) {
  ...
  schema.settings = normalizedSettings(node.fields).map(({ field, id }) => settingJson(field, id));
  const blocks = normalizedBlocks(node);
  if (blocks.length) {
    schema.blocks = blocks.map((b) => ({
      type: b.type,
      name: b.name,
      settings: b.settings.map(({ field, id }) => settingJson(field, id)),
    }));
...
// Render one setting as Liquid markup for a given access path (section.settings / block.settings).
function renderSetting(s, id, base, cls, pad) {
  const p = `${base}.${id}`;
  if (s.type === "S_IMAGE") {
    return [
      `${pad}{%- if ${p} != blank -%}`,
      `${pad}  <img src="{{ ${p} | image_url: width: 1600 }}" alt="${s.name}" loading="lazy">`,
      `${pad}{%- endif -%}`,
    ];
  }
  if (s.type === "S_CHECKBOX") return [`${pad}{%- if ${p} -%}<!-- ${s.name} on -->{%- endif -%}`];
  if (s.type === "S_RICHTEXT") return [`${pad}<div class="rte">{{ ${p} }}</div>`];
  return [`${pad}<span class="${cls}__${id}">{{ ${p} }}</span>`];
}
...
function buildSectionLiquid(node) {
  ...
  normalizedSettings(node.fields).forEach(({ field, id }) => {
    L.push(...renderSetting(field, id, "section.settings", handle, "  "));
  });

  const blocks = normalizedBlocks(node);
  if (blocks.length) {
    L.push(``, `  {%- for block in section.blocks -%}`);
    L.push(`    <div class="${handle}__block" {{ block.shopify_attributes }}>`);
    L.push(`      {%- case block.type -%}`);
    blocks.forEach((b) => {
      L.push(`        {%- when ${JSON.stringify(b.type)} -%}`);
      if (b.settings.length === 0) {
        L.push(`          {%- comment -%} no settings {%- endcomment -%}`);
      } else {
        b.settings.forEach(({ field, id }) => {
          L.push(...renderSetting(field, id, "block.settings", `${handle}__${b.type}`, "          "));
        });
      }
    });
    L.push(`      {%- endcase -%}`);
...
export default function SchemaFlow() {
      ...
      n.fields.forEach((f) => {
        if (f.type === "REFERENCE" && f.targetNodeId && byId[f.targetNodeId]) {
          ...
          if (target && target.id !== n.id) {
            out.push({ id: f.id, from: n, to: target, label: dashOf(f.name), kind: "section" });
          }
        }
      });
    });
  ...
  const addObject = (category = "metaobject") => {
    ...
    if (category === "section") {
      ...
      n = { ...base, category, name: uniqueName(fallback, nodes, category), fields: [],
        ...(category === "metafield" ? { ownerType: "PRODUCT" } : {}) };
    }
  ...
  const addBlock = () => {
    ...
    const existing = selected.blocks || [];
    const taken = new Set(existing.map((b) => b.name));
    let name = "Block", i = 2;
    while (taken.has(name)) name = `Block ${i++}`;
    const blocks = [...existing, { id: uid("b"), name, fields: [field("Image", "S_IMAGE")] }];
  ...
  // Metafield definitions for metafieldDefinitionCreate. Reference-type fields are
  // skipped (they'd need the target's definition GID, which we can't resolve here).
  const toShopifyMetafields = useCallback(() => {
    return schema.metafieldDefinitions
      .filter((d) => d.type !== "metaobject_reference")
      .map((d) => {
        const def = { name: d.name, namespace: d.namespace, key: d.key, type: d.type, ownerType: d.ownerType, pin: true };
        if (d.validations) def.validations = d.validations;
        return def;
      });
  }, [schema]);
  ...
  const openDeploy = () => {
    const deployables = schema.metaobjectDefinitions.length
      + toShopifyMetafields().length
      + nodes.filter((n) => n.category === "section").length
      + nodes.filter((n) => n.category === "template").length;
    if (deployables === 0) {
      fireToast("Add a metaobject, metafield, section, or template before deploying", false);
      return;
    }
  ...
  const runLiveDeploy = async () => {
    ...
    const metafields = toShopifyMetafields();
    const themeFiles = buildThemeFiles();
    const total = jsonSchema.length + metafields.length + themeFiles.length;
    setDeployState({ phase: "running", step: `Deploying ${jsonSchema.length} metaobject${jsonSchema.length === 1 ? "" : "s"}, ${metafields.length} metafield${metafields.length === 1 ? "" : "s"} + ${themeFiles.length} theme file${themeFiles.length === 1 ? "" : "s"}…`, log: [], error: "" });

    try {
      const res = await fetch(DEPLOY_ENDPOINT, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ storeUrl: host, adminApiToken: token.trim(), jsonSchema, metafields, themeFiles }),
      });
              ...
              <div className="space-y-3">
                ...
                {CATEGORY[selected.category].kind === "data" && (
                  <>
                    ...
                    {selected.category === "metafield" && (
                      <div>
                        <label className="mb-1 block font-mono text-[9px] uppercase tracking-wider text-slate-500">Attaches to (owner)</label>
                        <select value={selected.ownerType || "PRODUCT"} onChange={(e) => patchSelected({ ownerType: e.target.value })}
                          className="w-full rounded-lg border border-slate-700 bg-slate-900/70 px-2.5 py-2 text-[13px] text-slate-100 outline-none focus:border-violet-500/60">
          ...
          <div data-bg="1" className="relative" style={{
            ...
            <svg className="pointer-events-none absolute inset-0" width={bounds.w} height={bounds.h}>
              <defs>
                ...
                <marker id="sfArrowAmber" markerWidth="9" markerHeight="9" refX="6.5" refY="3" orient="auto">
              ...
              {connections.map((c) => {
                ...
                const stroke = isSection ? "#f59e0b" : "#22d3ee";
                const textFill = isSection ? "#fbbf24" : "#67e8f9";
                const glow = isSection ? "rgba(245,158,11,0.45)" : "rgba(34,211,238,0.45)";
                return (
                  <g key={c.id}>
                    <path d={g.d} fill="none" stroke={stroke} strokeWidth="1.6" strokeOpacity="0.6"
                      strokeDasharray={isSection ? "5 4" : "none"}
                      markerEnd={isSection ? "url(#sfArrowAmber)" : "url(#sfArrow)"}
                    ...
                    <g transform={`translate(${g.mx},${g.my})`}>
                      <rect x={-Math.max(20, c.label.length * 3.6)} y="-9" rx="5"
                        ...
                        fill="#0a1626" stroke={stroke} strokeOpacity="0.45" />
                      <text x="0" y="3.5" textAnchor="middle"
                        style={{ fill: textFill, fontSize: 9.5, fontFamily: "ui-monospace,monospace" }}>{c.label}</text>
                    </g>
                  </g>
                );
              })}
      ...
      {modalOpen && (
        <DeployModal
          ...
          counts={{
            ...
            metafields: toShopifyMetafields().length,
...
function DeployModal({ host, storeUrl, setStoreUrl, token, setToken, showToken, setShowToken,
  ...
  const summary = [
    ...
    counts?.metafields ? `${counts.metafields} metafield${counts.metafields === 1 ? "" : "s"}` : null,
